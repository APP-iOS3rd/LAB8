    ...
    var body: some View {
        ScrollView(.vertical, showsIndicators: false) {
            ...
            LazyVStack(spacing: 20) {
                ForEach(datas.filter {$0.title.contains(searchText)}) { data in
                    NavigationLink(destination: Text("Detail View")) {
                        FeedSearchCell(title: data.title, thumbnailUrl: data.thumbnailUrl)
                    }
                }
            }
            .padding()
...
//#Preview {
//    FeedListView(searchText: <#Binding<String>#>)
//}
